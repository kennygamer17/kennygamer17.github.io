...
struct estadisticas_personaje
{
    ...
    int vida_maxima;
    int vidas;
    int oleadas_sobrevividas;
    int enemigos_matados;
    int enemigos_sobrevividos;
    double golpe_total_efectuado;
    double golpe_total_recibido;
};
...
int main(){
    srand(time(NULL));
    bool lobby = true,personaje_creado_modo_historia = false,personaje_creado_modo_creativo = false,creativo = true,historia = true;
    ...
    while(lobby){
        ...
        switch(eleccion){
            ...
            puntos_de_carga();
            if(!personaje_creado_modo_historia){
                creacion_del_personaje(modo_seleccionado);
                personaje_creado_modo_historia = true;
            }
            while(historia){
            ...
            puntos_de_carga();
            if(!personaje_creado_modo_creativo){
                creacion_del_personaje(modo_seleccionado);
                creativo_configuracion(modo_seleccionado);
                personaje_creado_modo_creativo = true;
            }
            while(creativo){
                inventario_verificador(modo_seleccionado);
                modo[modo_seleccionado].personaje.estadisticas.estado = gestor_vida(modo_seleccionado);
